...
def load_model() -> Tuple[object, Dict, str]:
    ...
    if framework == "pytorch" and TORCH_AVAILABLE:
        ...
        checkpoint = torch.load(
            model_path / "disease_classifier.pt", map_location="cpu"
        )
        ...
        raise RuntimeError(
            f"Cannot load {framework} model - dependencies not available"
        )
...
def classify_image_pytorch(
    model: nn.Module, image: np.ndarray, label_mapping: Dict
) -> Dict:
    ...
    top_probs, top_indices = probabilities.topk(
        min(3, len(label_mapping["idx_to_label"]))
    )
    top_predictions = [
        {
...
def classify_image_sklearn(model, image: np.ndarray, label_mapping: Dict) -> Dict:
...
def get_treatment_recommendation(predicted_class: str) -> Dict:
    ...
    for key, value in TREATMENT_RECOMMENDATIONS.items():
        if (
            predicted_class.lower() in key.lower()
            or key.lower() in predicted_class.lower()
        ):
            return value
...
def classify_images(
    model, image_path: str, label_mapping: Dict, framework: str
) -> List[Dict]:
...
def run_inference(input_path: str, output_file: str):
    ...
    output = {
        ...
        "summary": {
            ...
            "diseased": sum(
                1 for r in results if not r.get("is_healthy", True) and "error" not in r
            ),
    ...
    # Critical alerts
    critical = [
        r for r in results if r.get("treatment", {}).get("severity") == "critical"
    ]
    if critical:
    ...
    logger.info(
        f"Summary: {output['summary']['healthy']} healthy, "
...
def main():
    parser = argparse.ArgumentParser(description="Classify crop diseases from image")
    ...
    parser.add_argument(
        "--output", "-o", type=str, required=True, help="Output predictions JSON file"
    )
